...
fn main() {
  ...
  let opt = WebBundlerOpt {
    ...
    scss_file: "../client/static/css/global.scss".into(),
    base_url: Some("/".into()),
...
mod web_bundler {
  ...
  pub struct WebBundlerOpt {
    ...
    pub tmp_dir: PathBuf,
    pub scss_file: PathBuf,
    /// Passed into the index.html template as base_url. Example template usage: `<base href="__WASM_BUILDER_BUILD_URL__">`
  ...
  /// <html lang="en">
  ///     <head>
  ///         <base href="__WASM_BUILDER__BASE_URL__">
  ///         <meta charset="utf-8">
  ///         <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  ///
  ///         __WASM_BUILDER_CSS__
  ///
  ...
  fn bundle_index_html(opt: &WebBundlerOpt) -> Result<()> {
    ...
    let mut index_html_content = fs::read_to_string(&src_index_path).with_context(|| {
      format!(
    ...
    let wasm_path =
      std::path::Path::new(opt.base_url.as_ref().unwrap_or(&"".to_string())).join("app.wasm");
    ...
    index_html_content = index_html_content.replacen("__WASM_BUILDER_JAVASCRIPT__", &javascript, 1);

    index_html_content = index_html_content.replacen(
      "__WASM_BUILDER_BASE_URL__",
      opt.base_url.as_deref().unwrap_or("/"),
      1,
    );
    ...
    let sass_options = sass_rs::Options {
      ...
      include_paths: vec![opt.scss_file.parent().unwrap().to_str().unwrap().into()],
    };
    let style_css_content = sass_rs::compile_file(&opt.scss_file, sass_options)
      .map_err(|e| anyhow!("Sass compilation failed: {}", e))?;

    let stylesheet = format!("<style>{}</style>", style_css_content);
    index_html_content = index_html_content.replacen("__WASM_BUILDER_CSS__", &stylesheet, 1);

    let mut html_minifier = html_minifier::HTMLMinifier::new();

    html_minifier.digest(index_html_content).unwrap();

    let index_html_content_minified = html_minifier.get_html();

    let dest_index_path = opt.dist_dir.join("index.html");
    fs::write(&dest_index_path, index_html_content_minified).with_context(|| {
      format!(
  ...
  fn bundle_app_wasm(opt: &WebBundlerOpt) -> Result<()> {
    ...
    let dest = opt.dist_dir.join("app.wasm");
